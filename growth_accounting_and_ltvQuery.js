          ...
          for (var valueKey in valuesObj) {
            if (valueKey == "cohortSize") {
              ...
              if (weeksBetween < 4) {
                ...
                newRow["04 Week LTV"] = Number((valuesObj[valueKey] / row[key]['cohortSize']).toFixed(2)); //want average 
              }
            } 
            else {
              var accumulatedValue = calculateTotalEventsForKey(valueKey, valuesObj);
              if (accumulatedValue) { //meaning total events are not null
                newRow[valueKey] = Number((accumulatedValue / row[key]['cohortSize']).toFixed(2)); //want average 
              } else {
